...
/*   Updated: 2023/02/09 04:34:32 by orekabe          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	*texture_circle(t_point *point, t_obj *obj)
{
	...
	res = (1 + atan(dot_prod_c(obj->ref.j, x)
				/ dot_prod_c(obj->ref.k, x)) / M_PI) * obj->diameter;
	res2 = sqrt((dot_prod_c(x, x)));
	a = (((int)(res * 10) % obj->img.height) + obj->img.height)
		% obj->img.height;
	b = (((int)(res2 * 10) % obj->img.width) + obj->img.width)
		% obj->img.width;
...
t_point	*checkerboard_circle(t_point *point, t_obj *obj)
{
	...
	x = sub_c(obj->coor, point->point);
	res = (1 + atan(dot_prod_c(obj->ref.j, x) / dot_prod_c(obj->ref.k, x))
			/ M_PI) * obj->diameter;
	res2 = sqrt((dot_prod_c(x, x)));
