...
/*   Updated: 2023/02/09 01:04:35 by nhanafi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bump_cal(double res, t_coordinates *direction, t_coordinates *normal)
{
	...
	tmp = prod_c((res - round(res)) / 2 + (double)(rand()%110 - 50)/1000, *direction);
...
t_point	*texture_square(t_point *point, t_obj *obj)
{
	...
	if (obj->img.sqsize)
	{
		bump_cal(-res * obj->img.sqsize / obj->img.height, &obj->ref.k, &point->normal);
		bump_cal(-res2 * obj->img.sqsize / obj->img.width , &obj->ref.j, &point->normal);
